    ...
    if user == 'poster'
	    ...
	    if dispute.present?
			if dispute.status = "refund" && dispute.status = "charge"
				...
				"Already refunds | #{link_to "Charge extra money", charge_to_poster_dispute_path(booking_id) }".html_safe
			elsif dispute.status = "charge"
				"#{link_to "Sending money", send_money_to_poster_dispute_path(booking_id)}  | Already Charged".html_safe 
			end	
		else
				"#{link_to "Sending money", send_money_to_poster_dispute_path(booking_id)} |  #{link_to "Charge extra money", charge_to_poster_dispute_path(booking_id)}".html_safe
